	...
	with open(filename, 'r') as f:
		...
		while True:
			...
			getTexFileDepMatch(dep, line, \
				'\\\\documentclass' + match_opt_arg, ext = 'cls', optional = True)
			# style files
			getTexFileDepMatch(dep, line, \
				'\\\\usepackage'+match_opt_arg, ext = 'sty', optional = True)
			# include | input
			getTexFileDepMatch(dep, line, \
			...
			getTexFileDepMatch(dep, line, \
				...
				ext = ['png', 'jpg', 'gif'], extdef = IMG_EXT_DEF)
...
def getTexFileDepMatch(dep, line, regex, \
	...
	if m is None: return
	f = m.group(1)
	if f == '' or (invalid is not None and re.match(invalid, f) is not None): return
	# add the directory
...
if __name__ == '__main__':
	...
	for file in sys.argv[1:]:
		...
		dd |= set(d)
		dd.add(file)
	accept = lambda f: os.path.splitext(f)[1] in ['.' + ext for ext in ['bib', 'cls', 'tex']]
